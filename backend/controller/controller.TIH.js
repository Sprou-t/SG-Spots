...
export const uploadTihDataToMongoAndS3 = async (req, res) => {
    try {
        // Fetch TIH data
        const tihData = await fetchTIHData('events');
                    ...
                    const imagePromises = tihObject.images.map(
                        async (image) => {
                            ...
                            try {
                                ...
                                await uploadImageToS3('tihImages', imageUuid, imageData); // upload to S3
                                return {
                                    success: true,
                                    imageUuid: imageUuid,
    ...
    try {
        ...
        for (const tihObject of tihData) {
            ...
            for (const imageUuid of tihObject.imagesUuid) {
                ...
                const extractedImgInfo = extractImageFromS3('tihImages', imageUuid);
...
export const retrieveSingleTihDataFromMongoAndS3 = async (req, res) => {
    ...
    try {
        ...
        for (const imageUuid of tihObject.imagesUuid) {
            ...
            const extractedImageFromAws = extractImageFromS3('tihImages', imageUuid);
            tihObjectForClientRender.images.push(extractedImageFromAws);
        }
        return res.status(200).json(tihObjectForClientRender);
    } catch (err) {
        return res.status(500).json(
            'failed in retrieving tih object from mongo and aws'
        );
    }
};
